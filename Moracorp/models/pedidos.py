...
class Pedidos(models.Model):
    ...
    '''Indicamos que cuando se necesite un nombre, se usara el atributo n_pedido'''
    _rec_name="n_pedido"
    ...
    direccion_cliente = fields.Char(string = 'Direccion del Cliente',compute="datos_cliente",store=True)
    telefono_cliente = fields.Char(string = 'Telefono del Cliente',compute="datos_cliente",store=True)
    email_cliente = fields.Char(string = 'Email del Cliente',compute="datos_cliente",store=True)
    ...
    # Aplicamos una restriccion sobre el campo numero de pedido para que sea unico
    _sql_constraints = [
        ('n_pedido_uniq', 'UNIQUE (n_pedido)', 'El Numero de Pedido debe ser único.')
    ]

    '''En funcion al campo venta se rellenaran automaticamente los campos referentes al cliente y a la fecha de venta
    del pedido que estamos tramitando'''
    @api.depends('venta')
        ...
        for pedidos in self:
            for recordventa in self.env['moracorp.ventas'].search([]):
                if pedidos.venta.n_transaccion == recordventa.n_transaccion:
                    ...
                    pedidos.direccion_cliente = recordventa.cliente.direccion
                    pedidos.telefono_cliente = recordventa.cliente.telefono
                    pedidos.email_cliente = recordventa.cliente.email
                    pedidos.fecha_expedicion = recordventa.fecha_compra
            
            
    # Aplicamos una restriccion al campo fecha_recepcion para que la fecha de recepcion no sea posterior a la fecha actual
    ...
    def check_fecha_recepcion(self):
        for record in self:
            if record.fecha_recepcion and record.fecha_recepcion < record.fecha_expedicion:
                raise models.ValidationError('Error la Fecha de Recepcion no puede ser anterior a la Fecha de Expedicion.')
