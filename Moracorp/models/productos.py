...
class Productos(models.Model):
    ...
    # Saca las compras que se han realizado de un producto en particular a partir del campo posible_compra
    @api.depends('posible_compra')
    def productos_compra(self):
        for record in self:
            record.compra = record.posible_compra

    # Saca las ventas que se han realizado de un producto en particular a partir del campo posible_venta
    @api.depends('posible_venta')
    def productos_venta(self):
        for record in self:
            record.venta = record.posible_venta
    
    # Saca la cantidad que hay en stock de un producto en particular a partir de los campos de compra y venta
    @api.depends('compra','venta')
    def stock(self):
        for record in self:
            record.cantidad = record.compra - record.venta
    
    # Saca los costes de un producto en particular a partir del campo posible_coste
    @api.depends('posible_coste')
    def producto_costes(self):
        for record in self:
            record.costes = record.posible_coste

    # Saca los ingresos de un producto en particular a partir del campo posible_ingreso
    @api.depends('posible_ingreso')
    def producto_ingresos(self):
        for record in self:
            record.ingresos = record.posible_ingreso

    # Saca los beneficios de un producto en particular a partir de los campos ingresos y costes
    ...
    def actualizar_stock(self):
        for recordproducto in self.env['moracorp.compras'].search([]):
            recordproducto.actualizoRegistrosCompras()
